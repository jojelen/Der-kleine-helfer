...
def print_ingredients(ingredients, f=sys.stdout, html=False, title="Ingredients"):
    ...
    for ing in ingredients:
        ...
        if ing["quantity"] != -1:
            print_paragraph(
                "{}: {} {}".format(ing["name"], ing["quantity"], ing["unit"]), f, html
            )
        else:
...
class CookBook:
    ...
    def print_recipes(self, idxs, f=sys.stdout, html=False):
        ...
        for idx, i in enumerate(idxs):
            ...
            print_href("{}.) {}".format(idx + 1, self.get_recipe_title(i)), f, url)
    ...
    def copy_tmp_files(self, idxs):
        ...
        for i in idxs:
            if self.df.loc[i, "pdf"] != "":
                shutil.copyfile(
                    os.path.join(self.dataDir, self.df.loc[i, "pdf"]),
                    os.path.join(tmp_dir, self.df.loc[i, "pdf"]),
                )
